  ...
  "A Consul-based service locator" should {
    "allow lookup of a registered service" in withServiceDiscovery { locator =>
      client =>
        ...
        val registeredUrl: Option[URI] = Await.result(locator.locate("service-1"), testTimeoutInSeconds seconds)
    ...
    "allow lookup of a service even if the same instance has been registered twice" in withServiceDiscovery { locator =>
      client =>
        ...
        val registeredUrl = Await.result(locator.locate("service-2"), testTimeoutInSeconds seconds)
    ...
    "return CompletableFuture[Empty] for lookup of services that aren't registered" in withServiceDiscovery { locator =>
      client =>
        val registeredUrl = Await.result(locator.locate("non-existing-service"), testTimeoutInSeconds seconds)
        registeredUrl shouldBe Option.empty[URI]
    }
